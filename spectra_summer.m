...
% create mask for summer data
m = month(t_uniform);
summerIdx = (m >= 5 & m <= 8);

% select relevant data
t_summer = t_uniform_num(summerIdx);
tem1_interp = interp1(matrix(:,1), matrix(:,2), t_summer, 'linear');
tem6_interp = interp1(matrix(:,1), matrix(:,3), t_summer, 'linear');
...
exportgraphics(gcf, 'figs/psd_plot_summer.png', 'Resolution', 300);